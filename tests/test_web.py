...
class FakeSupervisor:
    def __init__(self) -> None:
        ...
        values["chromecast"]["uuid"] = "12345678-1234-4234-8234-123456789abc"
...
async def test_untrusted_host_is_rejected_for_read_endpoints(
    ...
    client, _supervisor, _origin = management
    response = await client.get("/api/history", headers={"Host": "attacker.example"})
...
async def test_discovery_controls_and_reconnect_use_narrow_runtime_operations(
    ...
    control = await client.post(
        "/api/controls/stop", headers=headers, json={"request_id": "stop-1"}
    )
    reconnect = await client.post("/api/reconnect", headers=headers, json={})
...
async def test_history_is_opaque_and_thumbnail_rejects_arbitrary_ids(
    ...
    current = await client.get(f"/api/history/{EVENT_ID}/thumbnail")
    arbitrary = await client.get("/api/history/12345678-1234-4234-8234-123456789abc/thumbnail")
